...
### 🚀 Implementations
* **Svelte 5 + Tauri v2 Desktop App (Primary):** Located under `src/tauri/`. Built with **Svelte 5** (runes: `$state`, `$derived`, `$effect`), **TypeScript**, **Tauri 2**, and **Vite**, featuring a Rust math evaluation backend, local SQLite storage by default, and optional Supabase sync.
...
* **Local-First Storage:** Transactions are stored by default in an on-device **SQLite** database (`netcalc.db`) on both the Flutter and Tauri apps, so they work fully offline.
* **Optional Cloud Sync:** Supabase can optionally be enabled as the storage backend from each app's Settings page; if it can't be reached, they fall back to local SQLite.
